...
export class CourseController {
  ...
  @Get()
  async get(): Promise<CreateCourseDto[]> {
    return await this.service.findAll();
  }

  @Get('/:id')
  async getOne(@Param() param): Promise<CreateCourseDto> {
    return await this.service.findOne(param.id);
  }

  @UseGuards(AuthGuard)
  @Post()
  async post(@Body() course: CreateCourseDto): Promise<CreateCourseDto> {
    return await this.service.create(course);
  }

  @UseGuards(AuthGuard)
  @Delete('/:id')
  async delete(@Param() param): Promise<CreateCourseDto> {
